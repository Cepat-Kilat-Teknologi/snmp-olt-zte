...
**chi v5** (this repo + genieacs-relay); **Fiber v2** (freeradius-api, write-olt-zte).
The adapter standard mandates JSON format + HTTP contract + logging schema, not the framework.
Migration chi↔Fiber is a significant refactor with little upside — SNMP/SSH dominate all adapter workloads.

Fiber→chi porting: `fiber.Ctx` → `http.ResponseWriter + *http.Request`,
`c.Locals("request_id")` → `reqctx.RequestIDFromContext(r.Context())`,
`c.IP()` → `middleware/audit.go` `clientIP` helper (X-Forwarded-For aware).
Both chi adapters use the **leaf-package `reqctx` pattern** to avoid
the utils ↔ middleware ↔ logger import cycle.
...
| Security headers | ✅ | `securityHeadersMiddleware` (always active) |
| Body size limit | ✅ | Request body limit middleware |
